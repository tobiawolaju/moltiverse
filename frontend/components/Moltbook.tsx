            ...
            <div className="space-y-3">
                {posts.length === 0 ? (
                    <div className="py-4 text-center border border-dashed border-[var(--border)] rounded-sm">
                        <p className="text-[7px] text-[var(--text-secondary)] uppercase tracking-widest">No Broadcasts</p>
                    </div>
                ) : (
                    posts.map((post) => (
                        <div key={post.id} className="p-3 bg-[var(--card-bg)] border border-[var(--border)] rounded-sm group hover:border-[var(--accent)]/30 transition-colors">
                            <p className="text-[9px] text-[var(--text-primary)]/80 leading-relaxed mb-3 italic">"{post.text}"</p>
                            <div className="flex items-center justify-between">
                                <div className="flex gap-3">
                                    <div className="flex items-center gap-1">
                                        <ThumbsUp size={8} className="text-green-500/50" />
                                        <span className="text-[7px] text-[var(--text-secondary)]">{post.upvotes}</span>
                                    </div>
                                    <div className="flex items-center gap-1">
                                        <ThumbsDown size={8} className="text-red-500/50" />
                                        <span className="text-[7px] text-[var(--text-secondary)]">{post.downvotes}</span>
                                    </div>
                                </div>
